  ...
  -> cuvs::neighbors::ivf_rabitq::index<IdxT>
{
  ...
  static rmm::mr::managed_memory_resource s_ns_managed;   // PATCH: UVM for oversized build
  rmm::device_async_resource_ref big_memory_resource =
    rmm::device_async_resource_ref{s_ns_managed};
  ...
  if (!dataset_on_device) {
    ...
    if (params.force_streaming) {  // PATCH: disabled auto-streaming (broken on sm_70); use managed non-streaming
      use_streaming    = true;
